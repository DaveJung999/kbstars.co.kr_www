...
// 04/04/20 박선민 mime_content_type() 이용하여 정확한 mime-type를
// 04/06/17 박선민 해더 샘플 추가
// 04/07/12 박선민 bugfix - 214L exit; 추가
//=======================================================
...
//=======================================================
	...
	// 업로드 기본 디렉토리 설정
	$dbinfo['upload_dir'] = trim($dbinfo['upload_dir']) ? trim($dbinfo['upload_dir']) . "/{$SITE['th']}{$prefix}_{$dbinfo['db']}" : "{$thisPath}/../../sboard4/upload/{$SITE['th']}{$prefix}_{$dbinfo['db']}";
...
//	if($dbinfo['table_name'] != $dbinfo['db']) $sql_where=" table='{$dbinfo['db']}' "; // $sql_where 사용 시작
