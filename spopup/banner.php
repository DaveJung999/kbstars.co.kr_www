...
if(!$total=db_count()) {	// 게시물이 하나도 없다면...
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){
		...
		for($j=$i; ($j-$i < $dbinfo['row_pern']) && ($j < $total); $j++) { // 한줄에 여러값 출력시 루틴
			...
			$href['list']	= "list.php?db={$_GET['db']}";
			$href['download']	= "download.php?db={$_GET['db']}&uid={$list['uid']}";
			...
			elseif (strtolower(substr($list['upfiles']['upfile']['name'], -3)) == 'swf') $list['cur_banner'] =	"<object classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' codebase='http://active.macromedia.com/flash4/cabs/swflash.cab#version=4,0,0,0' width='200' height='65'>
										<param name='movie' value='{$href['download']}'>	<param name='play' value='true'>	<param name='loop' value='true'>	<param name='quality' value='high'>
										<embed src='{$href['download']}' play='true' loop='true' quality='high' pluginspage='http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash' width='200' height='65'></embed></object>";
