...
function cateDeleteOK($table_cate, $sql_where_cate, $sql_set_cate,$table_data,$sql_where_data){
	...
	if(strlen($cateinfo['re']))
		$sql="update {$table_cate}
				SET re=concat( substring(re,1,length('{$cateinfo['re']}')-1),
					char(ord(substring(re,length('{$cateinfo['re']}'),1))-1 ),
					substring(re,length('{$cateinfo['re']}')+1) )
				WHERE num='{$cateinfo['num']}' 
				AND {$sql_where_cate}
				AND re like '" . substr($cateinfo['re'],0,-1) . "%'
				AND strcmp(re,'{$cateinfo['re']}')>= 0
			";
	else 
		$sql="update {$table_cate}
				SET num=num-1
