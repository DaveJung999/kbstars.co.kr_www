...
switch($_REQUEST['mode']){
	case 'write':
		...
		go_url($_REQUEST['goto'] ? $_REQUEST['goto'] : "{$thisUrl}/list.php?" . href_qs("pid={$_POST['pid']}&pname={$_POST['pname']}",$qs_basic));
		break;
	case 'reply':
	...
	case 'modify':
		...
		break;
	case 'delete':
		$goto = $_REQUEST['goto'] ? $_REQUEST['goto'] : "{$thisUrl}/list.php?" . href_qs("pid={$pid}&pname={$_GET['pname']}",$qs_basic);
...
function write_ok($table, $qs){
	global $dbinfo, $db_conn, $SITE, $_SESSION;
