...
$trs = db_query($tsql);
$tcnt = db_count($trs);
$tselect = "<option>:::: 팀선택 ::::</option><option value=list.php>전체</option>";
if($tcnt) {
	for($i = 0 ; $i < $tcnt ; $i++) {
		$tlist = db_array($trs);
		if ($tlist['tid'] == $_GET['team']) $sel = "selected";
		${"$tlist['tid']_name"} = $tlist['t_name'];		
		$tselect .= "<option value=list.php?team=$tlist['tid'] $sel >$tlist['t_name']</option>";
		$sel = "";
	}
}
?>
...
<?= $tselect ?>
		...
		<td align="right"><a href="write.php?mode=write"><strong>[선수등록]</strong></a>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;</td>
	</form>
	</tr>
</table>
<br />
...
?>
  <tr align="center" bgcolor="#FFFFFF">
	...
	<td><?= $list['p_seq'] ?></td>
	<td><a href="write.php?mode=modify&amp;team=<?=$_GET['team']?>&amp;pid=<?= $list['uid'] ?>">수정</a> <a onclick="return del()" href="ok.php?mode=delete&amp;pid=<?= $list['uid'] ?>">삭제</a></td>
  </tr>
  <?php
		}
	}else{
...
<table width="95%" border="0" align="center">
	<tr align="right">
		<td height="25"><a href="write.php?mode=write"><strong>[선수등록]</strong></a>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;</td>
	</tr>
</table>
<?php
echo $SITE['tail'] ?? '';
