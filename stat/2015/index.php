...
//=======================================================
	...
	while( $list=db_array($result) ){
		...
		if($list['g_home'] == 13){
			...
			$list['strLogo'] = "/images/2011/image/emble_{$list['g_away']}.jpg";
			...
			$list['strLogo'] = "/images/2011/image/emble_{$list['g_home']}.jpg";
			$list['strWin'] = ($list['home_score'] < $list['away_score']) ? "승" : "패";
		}
		...
		if($list['home_score'] or $list['away_score']){
			$list['strLogo'] = "<a href='2-read.php?gid={$list['gid']}'>".$list['strLogo']."</a>";
		}
...
<table width="760" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		<td width="305"><img src="/images/2011/image/sub_title_3_1.jpg" height="42" /></td>
		<td>&nbsp;</td>
		<td align="right" class="sitemap">KB STARS 경기 &gt; 스타즈 경기일정 및 결과</td>
	</tr>
	</table></td>
	</tr>
	<tr>
	...
	<tr>
		<td><table width="754" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td height="35"><form name="form" id="form">
...
//echo "$season['s_start'] < $_GET['date'] -- $_gDate";
	...
	if(date("Ym", $season['s_start']) < date("Ym", $_gDate)){
		...
		$prevHref = " onclick=\"javascript:window.location.href='index.php?date=".$prevMonthTmp."'\" style=\"cursor:pointer\"" ;
	}
	if(date("Ym", $season['s_end']) > date("Ym", $_gDate)){
		// 다음월
		$nextMonthTmp = date("Y-m-01",strtotime("+1 month",$_gDate));
		$nextHref = " onclick=\"javascript:window.location.href='index.php?date=".$nextMonthTmp."'\" style=\"cursor:pointer\"" ;
	}
...
?>
	...
	<tr>
		<td><img src="/images/2011/image/schedule.jpg" width="754" height="46" /></td>
	</tr>
	</table>
		<table width='754' border='1' bordercolor='#dbdbdb' style='border-collapse:collapse'>
<?php
// for문 초기값 정의
...
//echo "$intThisYear-$NowThisYear == 0 and $intThisMonth-$NowThisMonth == 0 and $intPrintDay-$intThisDay";
				...
				echo "
					<table width='101' border='0' cellspacing='0' cellpadding='0'>
						<tr>
						<td class='font_notice1'><span class='schedule'><span class='point_pink'><span class='board_title'>$strThisDay</span></span></span></td>
						<td class='font_notice1' align='right'>{$outIcon[$intcday]}</td>
						</tr>
						<tr>
...
?>
	...
	<tr>
	<td><span style="padding-left:10px;"><img src="/img/h_a_t.gif" width="172" height="16" border="0" /></span></td>
	</tr>
</table>
<?php echo $SITE['tail']; ?>
